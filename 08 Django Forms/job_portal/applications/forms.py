...
class JobApplicationForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'cover_letter':forms.Textarea(attrs={'class':'form-control', 'placeholder':"Write a cover letter...", 'rows':4}),
    ...
    def clean_resume(self):
        ...
        if resume:
            if resume.size > 2 * 1024 * 1024:
                raise forms.ValidationError("The file size should not exceed 2MB")
            if not resume.name.endswith(('.pdf', 'doc', 'docx')):
                raise forms.ValidationError("Only PDF and Doc files are allowed")
        return resume
